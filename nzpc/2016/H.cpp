...
void solve() {
	...
	map<char, ll> points;
	string code = "SQALFDE";
	VLL vals = {10, 5, 7, -8, 4, -5, -10};
	rep(i,0,7) {
		points[code[i]] = vals[i];
	}
	...
	rep(i,0,m) {
		...
		M[id].points += points[c];
	}

	vector<mp> lo = {{inf,0,""}}, hi = {{-inf,0,""}};
	fe(p, M) {
		mp cur = p.second;
		if (p.second.points == lo[0].points) {
			lo.pb(cur);
		} else if (cur.points < lo[0].points) {
			lo = {cur};
		}

		if (cur.points == hi[0].points) {
			hi.pb(cur);
		} else if (cur.points > hi[0].points) {
			hi = {cur};
		}
	}

	sort(all(lo), [](mp & a, mp & b) {
		return a.id < b.id;
	});
	sort(all(hi), [](mp & a, mp & b) {
		return a.id < b.id;
	});
