...
ll sum(ll ind, ll owe);

ll finish(ll ind, ll owe) {
	// once owe gets larger than += 160, itll grow too big
	if (abs(owe) >= buf) return 0;
	if (dp[ind][owe+buf] != -1) return dp[ind][owe+buf];
	...
	ll hi = sum(ind+1, owe*31 + A[ind] - 32); 
	ll lo = sum(ind+1, owe*31 + A[ind] - 127);
	ll ans = hi - lo + mod;
	ans %= mod;

	return dp[ind][owe+buf] = ans;
}

ll sum(ll ind, ll owe) {
	if (owe > buf) return sum(ind, buf);
	if (owe <= -buf) return 0;
	if (sumdp[ind][owe+buf] != -1) return sumdp[ind][owe+buf];
	return sumdp[ind][owe+buf] = (sum(ind, owe-1) + finish(ind, owe)) % mod;
}
