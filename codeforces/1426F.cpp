...
ll get(ll i, ll letter, string & s) {
	if (letter == -1) {
		if (i == -1) return 1;
		return cnt[i];
	}
	...
	if (s[i] == '?') {
		ll ans = get(i-1, letter, s) * 3; // ignore 3 times
		ans += get(i-1, letter-1, s); // dont ignore, correct letter
		ans %= mod;
		return dp[i][letter] = ans;
	} else {
		ll ans = get(i-1, letter, s); // ignore once
		if (s[i] == letter + 'a') {
			ans += get(i-1, letter-1, s); // dont ignore
		}
		ans %= mod;
		return dp[i][letter] = ans;
	}
