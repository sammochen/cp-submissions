...
ll dfs(tnode *t, ll dep) {
	tnode* z = t->children[0];
	tnode* o = t->children[1];
	
	if (!z && !o) return 0;
	if (z && o) {
		ll ans = tp(dep);
		ans |= min(dfs(z, dep-1), dfs(o, dep-1));
		return ans;
	} else if (z) {
		return dfs(z, dep-1);
	} else {
...
void solve() {
	...
	fin(n);
	// make a trie
	// if there are two choices, pop a 1 and choose the minimum
	// otherwise, pop a 0
