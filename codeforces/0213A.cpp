...
ll f(ll i, vector<QLL> queues, VLL & A, VLL indeg, VVLL & E) {
	ll cnt = -1;
	while (queues[0].size() + queues[1].size() + queues[2].size()) {
		cnt++;
		while (queues[i].size()) {
			ll at = queues[i].front(); queues[i].pop();
...
void solve() {
	...
	ll cnt = inf;
	rep(i,0,3) upmin(cnt, f(i, queues, A, indeg, E));
