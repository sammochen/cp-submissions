...
struct kset {
	...
	ll size() {
		return sz(big) + sz(small);
	}

	ll minimum() {
		if (sz(small)) return *small.begin();
		if (sz(big)) return *big.begin();
		return inf;
	}
};
...
void solve() {
	ll n;
	fin(n);
	kset total, fire, lightning;
	rep(_,0,n) {
		ll tp, d;
		fin(tp, d);
		kset & temp = tp == 0 ? fire : lightning;
		...
		fire.setk(sz(lightning));
		total.setk(sz(lightning));
		lightning.setk(sz(lightning));

		ll options = 0;
		upmax(options, fire.bigsum); // use the lightning, one for each fire
		
		if (lightning.minimum() >= *total.big.begin()) {
			total.setk(sz(lightning) + 1);
			upmax(options, total.bigsum - lightning.minimum());
		} else {
			upmax(options, total.bigsum);
		}
	
		ll cur = total.sum + options;
