...
void dfs(ll at, VLL & order, VLL & vis, VVLL & revE) {
	vis[at] = 1;
	order.pb(at);
	
	fe(to, revE[at]) {
		if (!vis[to]) dfs(to, order, vis, revE);
	}
}
...
void solve() {
	...
	vector<SLL> temp(n);
	rep(i,1,n) {
		temp[p[i-1]-1].insert(p[i]-1);
		temp[q[i-1]-1].insert(q[i]-1);
	}

	VVLL E(n);
	rep(i,0,n) E[i] = VLL(all(temp[i]));
	...
	rep(at,0,n) {
		fe(to, E[at]) {
			if (id[at] == id[to]) continue;
			revE[id[to]].pb(id[at]);
		}
	}

	VLL order, vis(sz(scc));
	rep(i,0,sz(scc)) {
		if (!vis[i]) dfs(i, order, vis, revE);
	}
	reverse(all(order));
