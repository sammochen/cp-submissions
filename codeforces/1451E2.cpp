...
void solve() {
	...
	MLL ind;
	rep(i,0,n) {
		if (ind.find(Q[i]) != ind.end()) {
			// i and ind[Q[i]] is the same
			A[i] = q("AND", i, ind[Q[i]]);
			A[0] = A[i] ^ Q[i];
			rep(i,1,n) {
				A[i] = A[0] ^ Q[i];
	...
	// if not finished by here, all values are different.
	// find the one where A[0] ^ A[a] = n-1
	// now we know that A[0] & A[a] = 0

	// i have 2 queries left 
	ll a = 0;
	ll b = ind[n-1];
	ll r = b == 1 ? 2 : 1;
