...
ll win(ll x) {
	...
	// check if you can divide by any odd number
	ll z = x;
	while (z % 2 == 0) z /= 2;
	if (z == 1) return 0; // you cannot make any moves

	// now it is even and not 2
	// if it is divisible by 4 and above, then divide all the odd numbers, causing you to win
	if (x % 4 == 0) return 1;

	// now it is only divisible by 2
	// see how many odd factors can divide into x/2
	ll y = x/2;
	for (ll i = 3; i*i <= y; i++) {
		if (y % i == 0) return 1;
	}
...
void solve() {
	ll n;
	fin(n);
	string one = "Ashishgup", two = "FastestFinger";

	fout(win(n) ? one : two, nl);
}
