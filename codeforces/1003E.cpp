...
void solve() {
	...
	queue<PLL> free; // {dist, i}
	rep(i,0,d+1) {
		if (deg[i] > k) bye();
		free.push({i, max(i, d-i)});
	}

	rep(i,d+1,n) {
		PLL at;
		while (free.size()) {
			at = free.front(); free.pop();
			if (at.se == d) continue;
			if (deg[at.fi] == k) continue;
			break;
		}
		if (free.empty()) bye();
		...
		free.push({at.first, at.second});
		free.push({i, at.second + 1});
	}

	if (sz(ans) < n-1) bye();
	fout("YES", nl);
	fe(a, ans) fout(a.fi + 1, ' ', a.se + 1, nl);
}
