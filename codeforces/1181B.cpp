...
void solve() {
	...
	// you only need to try like 3 options
	// suppose 0123456
	// there are 6 points you can split at.

	vector<PLL> pos;
	rep(i,0,n-1) {
		ll leftlen = i+1;
		ll rightlen = n-leftlen;
		if (s[i+1] == '0') {
			continue;
		}

		pos.pb({max(leftlen, rightlen), i});
	}

	sort(all(pos));
	ll trials = min(sz(pos), 2LL); // try like 5 times lol a bit generous tbh
	...
	rep(t,0,trials) {
		...
		string cur = sum_s(left, right);

		if (ans == "") {
			ans = cur;
		} else if (lg(cur) > lg(ans)) {
