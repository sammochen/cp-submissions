...
void solve() {
	...
	ll cost = 0;
	for (PLL a : A) {
		multiset<ll>::iterator it = rn.upper_bound(a.first);
		if (rn.size() == 0 || it == rn.begin()) {
			cost += (x + (a.second - a.first) % mod * y % mod) % mod;
			cost %= mod;
		} else {
			it--;
			// greedy 
			ll keep = (a.second - *it + 1) * y;
			ll dont = x + (a.second - a.first) * y;
			
			cost += min(keep, dont) % mod;
			cost %= mod;

			rn.erase(it);
			
		}
		rn.insert(a.second + 1);
	}
	cout << cost << endl;
