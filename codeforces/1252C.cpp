...
void solve() {
	...
	rep(i,0,n) {
		preA[i+1] = preA[i] + (A[i] % 2);
		preB[i+1] = preB[i] + (B[i] % 2);
	}
	...
	rep(i,0,q) {
		...
		// path needs to be both all odd, or both all even
		ll x = preA[c] - preA[a-1];
		ll y = preB[d] - preB[b-1];

		if (x == 0 && y == 0) {
			fout("YES", nl);
		} else if (x == c-a+1 && y == d-b+1) {
			fout("YES", nl);
		} else {
			fout("NO", nl);
		}
