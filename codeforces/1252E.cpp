...
vector<PLL> get(VLL & A) {
	...
	rep(i,1,n) {
		if (A[i] == A[i-1]) {
			...
			ans[i] = {max(ans[i-1].fi - k, l), ans[i-1].se - 1};
		} else {
			ans[i] = {ans[i-1].fi + 1, min(ans[i-1].se + k, r)};
		}
	}
	return ans;
}
...
void solve() {
	...
	vector<PLL> one = get(A);
	reverse(all(A));
	vector<PLL> two = get(A);
	reverse(all(two));

	VLL ans(n);
	rep(i,0,n) {
		ll x = max(one[i].fi, two[i].fi);
		ll y = min(one[i].se, two[i].se);
		if (x > y || x > r || x < l) {
			fout(-1, nl);
			exit(0);
		}
