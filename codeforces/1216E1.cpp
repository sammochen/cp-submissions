...
// gets the sum of length of all numbers up to x
ll first(ll x) {
	ll p = 1; // current power
	ll sum = 0; // total ans
	ll add = 0; // sum of answers less than len
	for (ll len = 1; ; len++) {
		if (p * 10 - 1 < x) { // if x is larger
			ll amount = p * 10 - p;
			sum += add * amount + tri(amount) * len;
			add += amount * len;
		} else { // x is len 
			ll amount = x - p + 1;
			sum += add * amount + tri(amount) * len;
...
void solve() {
	...
	rep(i,0,q) {
		...
		x -= first(res-1); // take away that. now were left with one 123...res, and we are looking for the xth element
		...
		fout(to_string(ans)[x - second(ans-1)], nl);
	}
}
