...
void solve() {
	...
	multiset<ll> ms = {0}, vals;
	VVLL dels(n);
	VLL dp(n+1);
	rrep(i,n-1,0) {
		dp[i] = dp[i+1] + i + 1;

		// if the value is out of reach, delete it
		if (i + k + 2 <= n) ms.erase(ms.find(dp[i + k + 2]));
		// delete the dels
		fe(x, dels[i]) vals.erase(vals.find(x));
		if (sz(vals)) upmin(dp[i], *vals.begin());

		if (s[i] == '1') {
			ll val = ms.size() ? *ms.begin() + i + 1 : i + 1;
			upmin(dp[i], val);
			vals.insert(val);
			if (i - k - 1 >= 0) dels[i-k-1].pb(val);
		}
		ms.insert(dp[i]);
	}
