...
struct FASTINPUT {
	void f(string & s) { char buf[1000005]; ll a = scanf("%s", buf); a++; s = buf; }
...
ll good(ll a, ll b, ll n) {
	a &= ~tp(n);
	b &= ~tp(n);
	if (n == 1) return 1;
	else if (n == 2) {
		ll cnt = __builtin_popcount(a) + __builtin_popcount(b);
		return cnt % 2 == 1;
	} else {
		return good(a >> 1, b >> 1, n-1) && good(a, b, n-1);
	}
}
...
void solve() {
	...
	VVLL dp(n, VLL(tp(m), inf));
	rep(i,0,n) {
		rep(j,0,tp(m)) {
			ll cost = __builtin_popcount(B[i] ^ j);
			ll best = inf;
			if (i == 0) best = 0;
			else {
				rep(k,0,tp(m)) {
