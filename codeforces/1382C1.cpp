...
char flip(char c, ll x) {
	if (x % 2) return (c == '0') ? '1' : '0';
	return c;
}

char charat(string & s, ll i, fenwick & tree) {
	return flip(s[i], tree.sum(i));
}
...
void solve() {
	...
	VLL ans;
	rrep(x,n-1,0) {
		if (charat(a, j, tree) == b[x]) {
			j += d;
		} else {
			if (charat(a, i, tree) != charat(a, j, tree)) {
				ans.pb(1);
				tree.add(i, i, 1);
			} 
			ans.pb(x+1);
