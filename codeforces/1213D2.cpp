...
void solve() {
	...
	/*
	Knowing that each number can halve log times, we can record which numbers logging a number will cross
	e.g. 19: 19-9-4-2-1-0
	For each number, store all numbers that cross it, sorted by steps required
	Then, for each number, find the k closest ones 
	*/
	ll nax = 2e5 + 5;
	vector<multiset<ll>> ms(nax); // number -> steps
	VLL sum(nax); // keeps the running sum
