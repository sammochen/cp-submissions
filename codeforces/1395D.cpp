...
void solve() {
	...
	rep(i,0,d) lo.pb(0);
	// this might be a 2 pointer thing
	sort(rall(hi));
	sort(rall(lo));
	VLL hipre(1), lopre(1);
	fe(x, hi) hipre.pb(hipre.back() + x);
	fe(x, lo) lopre.pb(lopre.back() + x);

	ll ans = -inf;
	// enumerate the number of his taken
	rep(his, 0, sz(hi) + 1) {
		ll los = sz(lo);
		ll taken = his * d;
		ll notusedhi = sz(hi) - his;
		los -= (taken - notusedhi);

		upmin(los, sz(lopre) - 1);
		
		if (los < 0) continue;
		upmax(ans, lopre[los] + hipre[his]);
	}
