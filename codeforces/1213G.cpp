...
void merge(ll a, ll b) {
	if (same(a,b)) return;
	cnt += big[getp(b)] * big[getp(a)];
	big[getp(b)] += big[getp(a)];
	par[getp(a)] = getp(b);
}
...
void solve() {
	...
	// offline
	// sort edges by weight, and record answer as i go through. everytime i join 2 groups, answer adds by sizea * sizeb
	...
	ll ind = 0;
	rep(i,0,nax) {
		while (ind < n-1 && E[ind][0] <= i) {
			merge(E[ind][1], E[ind][2]);
			ind++;
		}
		ans[i] = cnt;
	}
