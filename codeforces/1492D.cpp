...
void solve() {
    VLL target(2);
    ll k;
    cin >> target[0] >> target[1] >> k;

    if (k == 0) {
        string res = string(target[1], '1') + string(target[0], '0');
        cout << "Yes" << endl;
        cout << res << endl;
        cout << res << endl;
        return;
    }
    // i now need at least 1 '0' and 2 '1'
    if (target[0] < 1 || target[1] < 2) {
        cout << "No" << endl;
        return;
    }
    ...
    target[0] -= 1;
    target[1] -= 2;
    if (target[0] + target[1] < k-1) {
        cout << "No" << endl;
        return;
    }

    string leftover = string(target[1], '1') + string(target[0], '0');
    string a = leftover.substr(0, k-1);
    string b = leftover.substr(k-1);
