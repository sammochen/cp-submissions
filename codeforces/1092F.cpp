...
ll dfs1(ll v, ll pr, ll d, VLL & weight, VLL & dist) {
	weight[v] = A[v];
	dist[v] = d;

	fe(u, E[v]) {
		if (u == pr) continue;
		weight[v] += dfs1(u, v, d+1, weight, dist);
	}
	return weight[v];
}

ll ans = -inf;
ll dfs2(ll v, ll pr, ll cur, VLL & weight) {
	upmax(ans, cur);

	fe(u, E[v]) {
		if (u == pr) continue;
		ll next = cur - weight[u] + total - weight[u];
		dfs2(u, v, next, weight);
	}
}
...
void solve() {
	...
	// choose some root, then move the root around
	VLL weight(n), dist(n);
	dfs1(0, -1, 0, weight, dist);

	ll cur = 0;
	rep(i,0,n) cur += A[i] * dist[i];
