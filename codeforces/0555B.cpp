...
void solve() {
	...
	rep(i,0,n-1) {
		ll op = A[i+1].fi - A[i].se;
		ll cl = A[i+1].se - A[i].fi;
		VLL gap = {cl, op, i};
		open[op].insert(gap);
		close[cl].insert(gap);
	...
	VLL ans(n-1, -1);
	fe(x, S) {
		fe(op, open[x]) cur.insert(op);

		fe(b, B[x]) {
			auto it = cur.lower_bound({b[0], -inf, -inf});
			if (it == cur.end()) {
				continue;
			}
			VLL gap = *it;
			...
			cur.erase(cur.find(gap));
			close[gap[0]].erase(close[gap[0]].find(gap));
		}

		fe(cl, close[x]) cur.erase(cur.find(cl));
	}
