...
ll get(VVLL & memory, ll k) { // returns how much memory i can get with max k convenience
    ll n = memory[0].size(), m = memory[1].size();
    VLL pre(n+1);
    rep(i,0,n) pre[i+1] = pre[i] + memory[0][i];

    ll sum = 0;
    ll ans = pre[min(n, k)];
    rep(i,0,m) {
        sum += memory[1][i];
        ll convenience = 2 * (i+1);
        if (convenience > k) break;

        ll left = k - convenience;
        upmin(left, n);
        upmax(ans, sum + pre[left]);
    }
    return ans;
}
