...
void dfs(ll at, ll prev, VLL & cnt, VLL & home, VLL & happy, VLL & good, VVLL & E, ll & ans) {
	cnt[at] = home[at];
	ll gsum = 0;
	fe(to, E[at]) {
		if (to == prev) continue;
		dfs(to, at, cnt, home, happy, good, E, ans);
		cnt[at] += cnt[to];
		gsum += good[to];
	}

	if ((cnt[at] + happy[at]) % 2) ans = 0;
	good[at] = (cnt[at] + happy[at]) / 2;
	if (good[at] < 0 || good[at] > cnt[at]) ans = 0;
	if (gsum > good[at]) ans = 0;
}
...
void solve() {
	...
	// condition for good - H must go down, 
	ll ans = 1;
	VLL cnt(n), good(n);
	dfs(0, -1, cnt, home, happy, good, E, ans);
