...
ll f(ll a, ll b, ll n, ll m, ll s, ll w, ll j) {
	if (a - j*w < 0) return -inf;
	ll i = min(n, (a - j*w) / s); // a takes this many s

	n -= i;
	m -= j;

	// b takes as many w as possible
	ll l = min(b/w, m);
	b -= l*w;
	ll k = min(b/s, n);
	b -= k*s;
