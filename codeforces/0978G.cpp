...
struct comp {
	bool operator() (const todo & a, const todo & b) {
		return a.b == b.b ? a.ind < b.ind : a.b < b.b;
	}
};
...
void solve() {
	...
	rep(i,0,n) {
		...
		if (S.size() == 0) continue; // nothing here
		if (ans[i]) continue; // already exam

		todo at = *S.begin();
		S.erase(S.begin());

		if (i >= examtime[at.ind]) {
			fout(-1, nl);
			return;
		}
		...
		S.insert({at.a, at.b, at.d-1, at.ind});
	}
