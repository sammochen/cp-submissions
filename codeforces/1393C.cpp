...
ll y(ll n, ll x, MLL & f) {
	set<PLL> cando;
	fe(p, f) cando.insert({p.se, p.fi});

	vector<PLL> ans(n);
	rep(i,0,n) {
		if (sz(cando) == 0) return 0;
		
		ans[i] = *(--cando.end());
		cando.erase(ans[i]);
		if (i-x >= 0 && ans[i-x].fi >= 2) cando.insert({ans[i-x].fi - 1, ans[i-x].se}); 
	}
...
void solve() {
	...
	// if i greedily choose the most frequent
	// take the most frequent first, then by 
}
