...
// sum that ends here with the biggest being big
ll get(ll ind, ll big) {
	if (ind == -1) return 0;
	if (A[ind] > big) return -inf;
	if (dp[ind][big+30] != -1) return dp[ind][big+30];

	ll ans = -inf;
	upmax(ans, get(ind-1, big) + A[ind]);
	upmax(ans, A[ind]);
