...
void solve() {
	...
	// greedily, you should end on the last day of some month. 
	// this is because its better than taking the 1.
	VLL pre(2*n + 1), pretri(2*n + 1);
	rep(i,0,2*n) {
		pre[i+1] = pre[i] + A[i];
		pretri[i+1] = pretri[i] + tri(A[i]);
	}

	ll ans = -inf;
	rep(i,n+1,2*n+1) {
		// find the kth value before
		ll ind = lower_bound(all(pre), pre[i] - x) - pre.begin();
		ll left = x - (pre[i] - pre[ind]);
		ll cur = pretri[i] - pretri[ind];
		cur += tri(A[ind-1]) - tri(A[ind-1] - left);
