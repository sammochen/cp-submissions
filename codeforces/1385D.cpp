...
ll get(ll i, ll j, ll c) {
	if (i == j) return (c + 'a') != s[i];

	ll m = (i+j)/2;

	ll L = pre[m+1][c] - pre[i][c];
	ll R = pre[j+1][c] - pre[m+1][c];
	ll len = m-i+1;

	ll ans = min(len - L + get(m+1, j, c+1), len - R + get(i, m, c+1));
	return ans;
...
void solve() {
	fin(n, s);
	pre.assign(n+1, VLL(26));
	
	rep(i,0,n) {
		rep(j,0,26) {
			pre[i+1][j] = pre[i][j] + (s[i] == j + 'a');
		}
	}
