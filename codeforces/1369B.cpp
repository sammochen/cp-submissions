...
ll match(char a, char c) {
	return a == c || a == 'X';
}
ll good(char a, char b) {
	return match(a, '1') && match(b, '0');
}
...
void solve() {
	...
	rep(i,0,n) {
		...
		while (ans.size() >= 2) { // take out 10 and replace with 'X'
			if (good(ans[ans.size()-2], ans[ans.size()-1])) {
				ans.pop_back();
				ans.pop_back();
				ans.push_back('X');
			} else break;
	...
	for (auto & c : ans) if (c == 'X') c = '0'; // change 'X' to 0
	fout(ans, nl);
}
