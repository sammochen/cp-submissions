...
// index i, 
ll get(ll i, ll k, ll cnt) {
	if (k == -1) return -inf;
	if (i == lg(s)) return 0;
	if (dp[i][k][cnt] != -1) return dp[i][k][cnt];

	ll ans = -inf;
	upmax(ans, get(i+1, k - (s[i] != t[0]), cnt+1)); // change to t[0]
	upmax(ans, get(i+1, k - (s[i] != t[1]), cnt) + cnt);
	upmax(ans, get(i+1, k, cnt)); // do nothing
	return dp[i][k][cnt] = ans;
}
