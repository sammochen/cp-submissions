...
ll get(ll i, ll j, ll k, ll width) {
    if (i < 0 || j < 0 || k < 0) return inf;
    if (i + width - 1 >= n || j + width - 1 >= n || k + width - 1 >= n)
        return inf;
    ...
    rep(x, 0, 2) {
        rep(y, 0, 2) {
            rep(z, 0, 2) {
                upmax(ans, get(i + x, j + y, k + z, width - 1));
            }
        }
    }
    return ans;
...
void solve() {
    ...
    rep(i, 0, n) {
        rep(j, 0, n) {
            rep(k, 0, n) {
                rep(width, 1, n + 1) {
                    ans[width] += (get(i, j, k, width) == width);
                }
            }
        }
    }
