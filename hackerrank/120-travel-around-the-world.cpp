...
void solve() {
    ...
    rep(i, 0, n) {
        cin >> A[i];
        upmin(A[i], c);  // no point getting more than c
    }
    rep(i, 0, n) cin >> B[i];

    // find some starting city
    ll s = 0;
    ll tank = 0;
    ll cnt = 0;
    ...
    // cool, so we know s is a start city. we can go backwards.
    VLL need(n, inf);
    need[s % n] = 0;

    rep(i, 1, n) {
        ll t = (s - i + 2 * n) % n;
        // i need b-a + next_need to get there
        need[t] = (B[t] - A[t]) + need[(t + 1) % n];
        upmax(need[t], 0);
    }
