...
void dfs(ll at, ll d, VLL& dist, VVLL& E) {
    if (dist[at] != -1) return;
...
VLL get_dist(ll at, VVLL& E) {
    VLL dist(sz(E), -1);
    dfs(at, 0, dist, E);
    return dist;
}

ll farthest(ll at, VVLL& E) {
    VLL dist = get_dist(at, E);
...
void solve() {
    ...
    VLL one = get_dist(options.fi, E);
    VLL two = get_dist(options.se, E);
    ll d = one[options.se];  // end to end dist
    ...
    rep(i, 0, q) {
        ...
        // first step is to get to one of the ends
        ll ans = max(one[v], two[v]);
        // remaining steps is going end to end
        ans += (k - 1) * d;
        cout << ans << endl;
    }
}
