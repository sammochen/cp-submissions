...
void solve() {
    ...
    // try sliding window
    // do O(n) sliding windows - each window is a different (i-j) value
    ...
    rep(diff, -n, n + 1) {
        ...
        if (i >= n || i + diff >= n) continue;

        // init sliding window and allow k bad items
        ll starti = i;
        ll endi = i;  // inclusive
        ll bad = s[starti] == t[starti + diff] ? 0 : 1;

        // j will be i + diff the entire time!!
        while (1) {
            if (bad > k) {
                // move left pointer
                if (s[starti] != t[starti + diff]) bad--;
                starti++;
            } else {
                // valid substring - update answer
                ll len = endi - starti + 1;
                upmax(ans, len);

                // move right pointer
                endi++;
                if (ob(endi, n) || ob(endi + diff, n)) break;
                if (s[endi] != t[endi + diff]) bad++;
            }
        }
    }
    cout << ans << endl;
