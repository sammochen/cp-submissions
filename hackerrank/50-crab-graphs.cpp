...
namespace SOLVE {	
	...
	void main() {
		...
		REP(i,0,m) {
			...
			deg[a]++;
			deg[b]++;
			addedge(E, W, hash(a,0), hash(b,1), 1);
			addedge(E, W, hash(b,0), hash(a,1), 1);
		}

		// add nodes get t to share
		REP(i,0,n) {
			addedge(E, W, at, hash(i,0), min(t, deg[i])); // t to each node to use
			addedge(E, W, hash(i,1), to, 1);
			//addedge(E, W, hash(i,0), hash(i,1), 1);
		}

		ll ans = dinic(E, W, at, to);
		cout << ans << endl;
