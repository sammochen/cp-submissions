...
void solve() {
    ...
    // B[i] - time left if starting from 0
    rep(i, 0, n) {
        B[i] -= i;
        if (B[i] <= 0) {
            LE.insert(B[i]);
    ...
    rep(i, 0, n) {
        ...
        // when you finish, move B[i] to the "back"

        // erase B[i]
        if (LE.find(B[i]) != LE.end()) {
            LE.erase(LE.find(B[i]));
        ...
        // insert B[i] - n
        LE.insert(B[i] - n);  // put it in LE
                              // by next iteration itll get moved if necessary
    }
