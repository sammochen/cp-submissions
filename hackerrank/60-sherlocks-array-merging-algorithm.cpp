...
ll get(ll ind, ll spaces) {
	if (ind == sz(A)) return 1;
	if (dp[ind][spaces] != -1) return dp[ind][spaces];

	ll ans = 0;
	rep(i,ind,n) {
		if (i != ind && A[i] < A[i-1]) break;
		ll spots = i - ind + 1;
		if (spots > spaces) break;

		// i to ind is sorted
		// now add them to the spaces that we have, 
		if (ind == 0) {
			ans += get(i+1, spots);
			ans %= mod;
		} else {
			ans += get(i+1, spots) * f(spaces) % mod * invf(spaces-spots) % mod;
			ans %= mod;
		}
	}
	dp[ind][spaces] = ans;
