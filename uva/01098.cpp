...
ll dfs2(ll i, ll j, VVLL & copy) {
	if (ob(i, n) || ob(j, m)) return 0;
	if (copy[i][j]) return 0;
	copy[i][j] = 1;

	ll cnt = 1;
	rep(x,0,4) {
		cnt += dfs2(i+di[x], j+dj[x], copy);
	}
	return cnt;
}

void dfs(ll i, ll j, ll steps, ll ind) {
	if (ob(i, n) || ob(j, m)) return;	
	if (vis[i][j]) return;

	if (steps == num[ind]) {
		if (i == target[ind].fi && j == target[ind].se) {
			ind++;
			if (ind == 4) {
				ans++;
				return;
		...
		// cannot step on a target if its not the turn
		rep(x,0,4) {
			if (i == target[x].fi && j == target[x].se) return;
		}
		
		ll d = abs(i - target[ind].fi) + abs(j - target[ind].se);
		if (d + steps > num[ind]) return;
	}

	// if i visit this spot, would i still be able to reach everywhere from the last node?
	VVLL copy = vis;
	copy[i][j] = 1;
	ll cnt = dfs2(0, 1, copy);
	if (cnt + steps != n*m) return;

	vis[i][j] = 1;
	rep(x,0,4) {
		dfs(i + di[x], j + dj[x], steps+1, ind);
	}
	vis[i][j] = 0;
}
