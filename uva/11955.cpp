...
big ncr(ll n, ll r) {
	big ans = 1;
	REP(i,n-r+1,n+1) ans = ans * i;
	REP(i,1,r+1) ans =  ans / i;
	return ans;
}

string mult(big x, string s) {
	if (x == 1) return s;
	return to_string(x) + "*" + s;
}

string pow(string s, ll k) {
	if (k == 1) return s;
	return s + "^" + to_string(k);
}

string term(big a, string b, ll c, string d, ll e) {
	return mult(a, pow(b, c)) + "*" + pow(d, e);
}
...
void solve() {
	...
	REP(i,0,s.length()) {
		...
		if (good) split[ptr].push_back(s[i]);
		else if (split[ptr].length() != 0) ptr++;
	}
	...
	REP(i,0,n+1) {
		if (i == 0) ans += pow(split[0], n);
		else if (i == n) ans += "+" + pow(split[1], n);
		else ans += "+" + term(ncr(n, i), split[0], n-i, split[1], i);
	}
