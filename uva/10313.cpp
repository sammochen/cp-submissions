...
ll get(ll v, ll cur, ll cnt) {
	if (v < 0) return 0;
	if (cnt < 0) return 0;
	if (cur == 301) {
		if (v == 0 && cnt == 0) return 1;
		return 0;
	}
	if (dp1[v][cur][cnt] != -1) return dp1[v][cur][cnt];

	dp1[v][cur][cnt] =  get(v-cur, cur, cnt-1) + get(v, cur+1, cnt);
	return dp1[v][cur][cnt];
}
...
ll get2(ll v, ll cnt) {
	...
	if (dp2[v][cnt] != -1) return dp2[v][cnt];
	dp2[v][cnt] = get2(v, cnt-1) + get(v, 1, cnt);
	return dp2[v][cnt];
}
