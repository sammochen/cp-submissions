...
ll get_length(ll i, ll j, ll d) {
	if (ob(i, n) || ob(j, m)) return 0;
	ll & ans = pre[i][j][d];
	if (ans != -1) return ans;
	ll ii = i + di[d], jj = j + dj[d];
	ans = A[i][j] == 0 ? 0 : 1 + get_length(ii, jj, d);
	return ans;
}

ll f(ll big, ll small) {
	return max(0LL, min(small, big/2) - 1);
}

void solve() {
	cin >> n >> m;
	A.assign(n, VLL(m, 0));
	pre.assign(n, VVLL(m, VLL(4, -1)));
	rep(i,0,n) {
		rep(j,0,m) {
			cin >> A[i][j];
		}
	...
	rep(i,0,n) {
		rep(j,0,m) {
			VLL hori = {get_length(i, j, 0), get_length(i, j, 1)};
			VLL vert = {get_length(i, j, 2), get_length(i, j, 3)};
			fe(h, hori) {
				fe(v, vert) {
					ans += f(h, v);
					ans += f(v, h);
