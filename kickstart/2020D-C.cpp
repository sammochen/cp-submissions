...
// dfs(0, E, path, acnt, bcnt, a, b);
void dfs(ll at, VVLL & E, VLL & path, VLL & acnt, VLL & bcnt, ll a, ll b) {
	path.pb(at);
	fe(to, E[at]) {
		dfs(to, E, path, acnt, bcnt, a, b);
	}

	acnt[at]++;
	bcnt[at]++;
	if (sz(path) - 1 - a >= 0) {
		acnt[path[sz(path)-1-a]] += acnt[at];
	}
	if (sz(path) - 1 - b >= 0) {
		bcnt[path[sz(path)-1-b]] += bcnt[at];
	}
	path.pop_back();
}
...
void solve() {
	...
	db ans = 0;
	rep(i,0,n) {
		double x = (db)acnt[i] / (db)n;
		double y = (db)bcnt[i] / (db)n;
		ans += x + y - x*y;
	}
